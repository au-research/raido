...
@Slf4j
public class OrcidServiceStub extends OrcidService {
    private final Long delayMilliseconds;
    public OrcidServiceStub(final Long delayMilliseconds) {
        super(null);
    ...
    public List<ValidationFailure> validate(String uri, String fieldId) {
        ...
        if (!uri.matches(regex)) {
            ...
            log.debug("simulate ROR validation check");
            ...
            if (areEqual(uri, NONEXISTENT_TEST_ORCID)) {
                failures.add(new ValidationFailure()
                        .fieldId(fieldId)
                        .errorType(INVALID_VALUE_TYPE)
                        .message(URI_DOES_NOT_EXIST)
                );
            } else if (areEqual(uri, SERVER_ERROR_TEST_ORCID)) {
                failures.add(new ValidationFailure()
                        .fieldId(fieldId)
                        .errorType(INVALID_VALUE_TYPE)
