  ...
  void mintRaidV1_ReturnsOk() throws Exception {
    ...
    try (MockedStatic<AuthzUtil> authzUtil = Mockito.mockStatic(AuthzUtil.class)) {
      ...
      mockMvc.perform(post("/raid/v1")
        ...
        .andExpect(jsonPath("$.contributors[0].positions[0].position", Matchers.is("https://github.com/au-research/raid-metadata/blob/main/scheme/contributor/position/v1/leader.json")))
  ...
  void listRaidsV1_ReturnsOk() throws Exception {
    ...
    try (MockedStatic<AuthzUtil> authzUtil = Mockito.mockStatic(AuthzUtil.class)) {
      ...
      mockMvc.perform(get("/raid/v1", handle)
        ...
        .andExpect(jsonPath("$[0].contributors[0].positions[0].position", Matchers.is("https://github.com/au-research/raid-metadata/blob/main/scheme/contributor/position/v1/leader.json")))
