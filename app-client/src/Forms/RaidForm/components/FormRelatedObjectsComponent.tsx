                                            ...
                                            <>
                                                <Stack direction="row" alignItems="flex-start" gap={1}>
                                                    <Grid container spacing={2}>
                                                        ...
                                                        <Grid item xs={12} sm={6} md={3}>
                                                            <pre>{JSON.stringify(controllerField?.value?.category?.[0])}</pre>
                                                            <TextField
                                                                select
                                                                value={Array.isArray(controllerField?.value?.category) ? controllerField?.value?.category?.[0]?.id : ""}
                                                                size="small"
                                                                fullWidth
                                                                ...
                                                                onChange={(event) => {
                                                                    ...
                                                                    if (Array.isArray(controllerField?.value?.category) && controllerField?.value?.category?.length > 0) {
                                                                        // Create a deep copy of the category array
                                                                        const updatedCategory = [...controllerField.value.category];
                                                            ...
                                                            >
                                                                {relatedObjectCategories.map((relatedObjectCategory) => (
                                                                    <MenuItem
                                                                        key={relatedObjectCategory.uri}
                                                                        value={relatedObjectCategory.uri}
                                                                    >
                                                                        {extractKeyFromIdUri(relatedObjectCategory.uri)}
                                                                    </MenuItem>
                                                                ))}
                                                            </TextField>
                                                        </Grid>
