...
export function ErrorDialogProvider(props: { children: React.ReactNode }) {
  const [errorState, dispatch] = React.useReducer(reducer, {
    dialogOpen: false,
    errors: [],
  });

  return (
    <errorDialogContext.Provider value={dispatch}>
      {props.children}
      <ErrorDialog
        dialogOpen={errorState.dialogOpen}
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <div>
          <ul>
            {filteredErrors.reverse().map((i, index) => {
              return (
                <li key={index}>
...
function SignInDetails() {
  ...
  let details = <Typography>You have not signed in tho the app.</Typography>;
  ...
  if (auth) {
    ...
    details = (
      <InfoFieldList>
        ...
        <InfoField
          id={"sessionExpiry"}
          label="Sign-in expiry"
          value={
            <Typography color={expiryFieldColor}>{formattedExpiry}</Typography>
          }
        />
